...
std::unique_ptr<StochasticEncoder<float>> MakeEncoder() {
  ...
  return std::unique_ptr<StochasticEncoder<float>>(new StochasticEncoder<float>(
...
BOOST_AUTO_TEST_CASE(avg_deviation) {
  ...
  const double values[] = {
      -(M_PI), -(M_E), -(M_SQRT2), -1.0,
      -0.01,   0.0,    0.01,       1.0,
      M_SQRT2, M_E,    M_PI,       std::numeric_limits<double>::infinity()};
