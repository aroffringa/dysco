...
public:
  ...
  /**
   * Rename the old column and create a new column with the default storage
   * manager. After all columns have been renamed, the operation should be
   * finished by a call to MoveColumnData() if @c true was returned.
   * @param columnName Name of column.
   * @param isWeight true if a weight (float) column should be created, or false
   * to create a complex column.
   * @returns @c true if the storage manager was changed.
   */
  bool InitColumnWithDefaultStMan(const std::string &columnName,
                                  bool isWeight) {
    ...
    if (dataManager == "DyscoStMan") {
      ...
      if (isWeight) {
        ...
        casacore::ArrayColumnDesc<float> columnDesc(columnName,
                                                    oldColumn.shape(0));
   ...
   * After all columns have been renamed, the operation should be finished by a
   * call to MoveColumnData() if @c true was returned.
   * @param columnName Name of column.
   * @param bitsPerComplex If a DyscoStMan needs to be constructed, set this
   * number of bits per data value.
   * @param bitsPerWeight If a DyscoStMan needs to be constructed, set this
   ...
   * This should be called for each changed column after all calls to the
   * PrepareReplacing...() method have been performed, and the data managers
   * have been added.
  ...
  template <typename T> void MoveColumnData(const std::string &columnName) {
    ...
    std::unique_ptr<casacore::ArrayColumn<T>> oldColumn(
        new casacore::ArrayColumn<T>(_ms, tempName));
   ...
   */
  static void Reorder(std::unique_ptr<casacore::Table> &ms,
...
private:
  ...
  template <typename T>
  void copyValues(casacore::ArrayColumn<T> &newColumn,
                  casacore::ArrayColumn<T> &oldColumn, size_t nrow) {
