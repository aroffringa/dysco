 ...
 * faster than the corresponding std::vector constructor, and has no overhead to
 * a manually allocated array.
 *
 ...
 * std::vector. This is true because of more strengent requirements on the
 * element's type.
 *
 ...
 * memmove, the @ref push_back() and @ref insert() methods are a bit faster than
 * the std::vector counterparts, at least on gcc 4.7.
 *
...
class uvector : private Alloc {
 ...
 public:
  ...
  iterator insert(const_iterator position, const Tp& item) {
    if (_end == _endOfStorage) {
      ...
      memmove(const_cast<iterator>(position) + 1, position,
              (_end - position) * sizeof(Tp));
      ++_end;
    }
  ...
  iterator insert(const_iterator position, size_t n, const Tp& val) {
    if (capacity() < size() + n) {
      ...
      memmove(const_cast<iterator>(position) + n, position,
              (_end - position) * sizeof(Tp));
      _end += n;
    }
  ...
  iterator erase(const_iterator position) {
    --_end;
    memmove(const_cast<iterator>(position), position + 1,
            (_end - position) * sizeof(Tp));
    return const_cast<iterator>(position);
  }
  ...
  iterator erase(const_iterator first, const_iterator last) {
    size_t n = last - first;
    _end -= n;
    memmove(const_cast<iterator>(first), first + n,
            (_end - first) * sizeof(Tp));
   ...
   */
  void swap(uvector<Tp, Alloc>& other) { swap(other, std::false_type()); }
 ...
 private:
  ...
  void deallocate(pointer begin, size_t n) {
    if (begin != 0) Alloc::deallocate(begin, n);
  }
  ...
  void enlarge_for_insert(size_t newSize, size_t insert_position,
    ...
    memcpy(newStorage + insert_position + insert_count,
           _begin + insert_position, (size() - insert_position) * sizeof(Tp));
    deallocate();
